...
import { calendarDate, bangkokRange } from '../lib/calendar.js';

const router = Router();
const querySchema = z.object({ semesterId: z.coerce.number().int().positive().optional(), from: calendarDate.optional(), to: calendarDate.optional() }).strict();
...
function period(input: z.infer<typeof querySchema>) {
  const range = bangkokRange(input.from, input.to);
  if (range.from >= range.to || range.to.getTime() - range.from.getTime() > 367 * 86400000) throw new ApiError(400, 'DATE_RANGE_INVALID', 'Date range is invalid');
  return range;
}
...
router.get('/dashboard', async (req, res) => {
  ...
  const activities = await db.execute(sql`SELECT ee.id, ee.type::text AS type, ee.class_id AS "classId", c.name AS "className", ee.occurred_at AS "occurredAt" FROM enrollment_events ee JOIN classes c ON c.id=ee.class_id WHERE ${classScope} ${semester} AND ee.occurred_at >= ${range.from} AND ee.occurred_at < ${range.to} ORDER BY ee.occurred_at DESC LIMIT 20`);
...
router.get('/search', async (req, res) => {
  ...
  const [classRows, subjectRows, departmentRows, userRows] = await Promise.all([
    ...
    db.execute(sql`SELECT s.id, s.name, 'subject' AS type FROM subjects s LEFT JOIN classes c ON c.subject_id=s.id WHERE ${classScope} AND (s.name ILIKE ${term} OR s.code ILIKE ${term}) GROUP BY s.id,s.name ORDER BY s.name,s.id LIMIT 8`),
    db.execute(sql`SELECT d.id, d.name, 'department' AS type FROM departments d LEFT JOIN subjects s ON s.department_id=d.id LEFT JOIN classes c ON c.subject_id=s.id WHERE ${classScope} AND d.name ILIKE ${term} GROUP BY d.id,d.name ORDER BY d.name,d.id LIMIT 8`),
